		...
		$(document).ready(function() {
			...
			$("#inputForm").validate({
				rules: {
					...
					episodeNumber:{required:true,digits:true,min:1},
					genreTreeIndex:"required",
					regionTreeIndex:"required",
					providerId:"required",
					ftpId:"required",
					videoFtpPath:"required",
					posterFtpPath:"required",
					productId:"required"
				},
				messages: {
					number: "不能为空,且为正整数",
					episodeNumber: "不能为空,且为正整数",
					genreTreeIndex:"不能为空",
...
<div id="yui-main">
<div class="yui-b">
<h2><s:if test="id == null">创建</s:if><s:else>修改</s:else>TV</h2>
<form id="inputForm" action="tv!save.action" method="post"><input type="hidden" name="id" value="${id}" />
<table class="noborder">
<tr>
		<td>开始序号:</td>
		<td><input type="text" name="beginTvNum"  size="40" id="beginTvNum" value="${beginTvNum}" /></td>
	</tr>
	<tr>
	...
	<tr>
		<td>剧集数:</td>
		<td><input type="text" name="episodeNumber" size="40" id="episodeNumber" value="${episodeNumber}" /></td>
	</tr>
	<tr>
		<td>风格:</td>
		<td><input type="text" name="genreTreeIndex" size="40" id="genreTreeIndex" value="${genreTreeIndex}" /></td>
	</tr>
	<tr>
		<td>区域:</td>
	...
	<tr>
		<td>预览（视频）FTP路径:</td>
		<td><input type="text" name="previewFtpPath" size="40" id="previewFtpPath" value="${previewFtpPath}" /></td>
	</tr>
	<tr>
		<td>海报FTP路径:</td>
