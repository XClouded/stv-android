...
<table id="contentTable">
	...
	<s:iterator value="page.result">
		<tr>
			...
			<td>
			<a href="movie!input.action?id=${id}">详情</a>&nbsp; 
			<c:if test='${state=="EDITABLE"}'>
				<a href="movie!updateToCensoring.action?id=${id}" onclick="return confirm('确定批量提交审核？')">批量提交审核</a>
			</c:if> <c:if test='${state=="CENSORING"}'>
				<a href="movie!updateToIngestable.action?id=${id}" onclick="return confirm('确定批量审核吗？')">批量审核</a>
			</c:if> <c:if test='${state=="INGESTABLE"}'>
				<a href="movie!updateToIngested.action?id=${id}" onclick="return confirm('确定批量注入吗？')">批量注入</a>
			</c:if> <c:if test='${state=="INGESTED"}'>
				<a href="movie!offline.action?id=${id}" onclick="return confirm('确定批量下线吗？')">批量下线</a>
			</c:if> <c:if test='${state=="ZOMBIE"}'>
				<a href="movie!delete.action?id=${id}" onclick="return confirm('确定删除吗？')">删除</a>
			</c:if>
			</td>
...
<div class="pagination"><%@ include file="/common/page.jsp"%> <a href="movie!input.action">增加电影</a></div>
</form>
</div>
</div>
</div>
