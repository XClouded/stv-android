	...
	<form id="mainForm" action="tv.action" method="get">
			...
			<table id="contentTable">
				...
				<s:iterator value="page.result">
					<tr>
						...
						<td>
						<a href="ktv!input.action?id=${id}">详情</a>&nbsp; 
						<c:if test='${state=="EDITABLE"}'>
						<a href="ktv!updateToCensoring.action?id=${id}" onclick="return confirm('确定批量提交审核？')">批量提交审核</a>
						</c:if>
						<c:if test='${state=="CENSORING"}'>
						<a href="ktv!updateToIngestable.action?id=${id}" onclick="return confirm('确定批量审核吗？')">批量审核</a>
						</c:if>
						<c:if test='${state=="INGESTABLE"}'>
						<a href="ktv!updateToIngested.action?id=${id}" onclick="return confirm('确定批量注入吗？')">批量注入</a>
						</c:if>
						
						<c:if test='${state=="INGESTED"}'>
						<a href="ktv!offline.action?id=${id}" onclick="return confirm('确定批量下线吗？')">批量下线</a>
						</c:if>
						<c:if test='${state=="ZOMBIE"}'>
						<a href="ktv!delete.action?id=${id}" onclick="return confirm('确定删除吗？')">删除</a>
						</c:if>
					   </td>
					</tr>
				</s:iterator>
		...
		<div class="pagination">
		    <%@ include file="/common/page.jsp" %>
				<a href="ktv!input.action">增加KTV</a>
		</div>
	</form>
	</div>
	</div>
