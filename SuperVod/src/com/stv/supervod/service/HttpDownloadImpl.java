...
public class HttpDownloadImpl {
	 ...
	 */
	public enum KeyEnum {
		id, title, name, assets, poster, categories, ratingBar, ratingScore1, ratingScore2, recommendedRank, director, actor, descrip, offeringId, assetId, state, episodeId, bandwidth, totalNum, type, startTime, volumes, play_mode, play_count, npt, serviceType, price, bookmark, runTime, treeIndex, user, ispsd, psd, category, order, num, page, channelId, date, keyword, list, success, result, is, operationCode, scale, singers, songs, playTimes
	}
	 ...
	 * @throws ErrorCodeException
	 */
	public Map<String, Object> downloadCommonList(String category, String type, int order, int num, int page, String url)
	 ...
	 * @throws ErrorCodeException
	 */
	public Map<String, Object> downloadOffingId(String assetId, String type) throws ClientProtocolException, IOException, JSONException,
			ErrorCodeException {
		Map<String, String> params = new HashMap<String, String>();
		params.put(KeyEnum.assetId.toString(), assetId);
		params.put(KeyEnum.type.toString(), type);
	 ...
	 * @throws ErrorCodeException
	 */
	public Map<String, Object> downloadSongsBySinger(String actor, int num, int page) throws ClientProtocolException, IOException, JSONException,
			ErrorCodeException {
		num = Integer.valueOf(num) == null ? 20 : num;
		page = Integer.valueOf(page) == null ? 1 : page;
	 ...
	 * @throws ErrorCodeException
	 */
	public Map<String, Object> downloadProgramList(String date, String channelId) throws ClientProtocolException, IOException, JSONException,
			ErrorCodeException {
	 ...
	 * @throws ErrorCodeException
	 */
	public Map<String, Object> downloadSearchProgramsList(String keyword, String type, int num, int page) throws ClientProtocolException,
	 ...
	 */
	/*
	 * public Drawable downloadPoster(String url) throws IOException { Drawable
	 * drawable = ImageServiceImpl.getImageDrawableByUrl(url); if (drawable ==
	 * null) { String dpath = Constant.download_dir; URL imageurl = new
	 * URL(url); HttpURLConnection conn = (HttpURLConnection)
	 * imageurl.openConnection(); conn.setConnectTimeout(3 * 1000);
	 * conn.setRequestMethod("GET"); conn.setDoInput(true); conn.connect();
	 * InputStream is = conn.getInputStream(); Bitmap bp =
	 * BitmapFactory.decodeStream(is); is.close(); String[] names =
	 * url.split("/"); String filename = ""; if (names.length > 0) { filename =
	 * names[names.length - 1]; } if (bp != null &&
	 * !ValidateUtil.isBlank(filename)) { FileOutputStream fos = new
	 * FileOutputStream(new File(dpath, filename));
	 * bp.compress(Bitmap.CompressFormat.PNG, 100, fos); fos.close(); drawable =
	 * Drawable.createFromStream(is, filename); } } return drawable;
	 * 
	 * }
	 */
