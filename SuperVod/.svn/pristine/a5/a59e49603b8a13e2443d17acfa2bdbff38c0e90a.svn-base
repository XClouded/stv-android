...
public class EpisodeDetail extends BaseActivity{
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		mTvInfoRow3 = (TextView) findViewById(R.id.tv_detail_tv_row3);
			...
			try {
				if( params[0] == EpiDetailOp.get_info ){
					...
					if( mEpisodeLists != null ){
						...
						mServiceInfo = httpDownload.downloadOffingId(assetId, TypeEnum.EPISODE.toString());
						MyVodServiceImpl myvodSer = new MyVodServiceImpl();
						mIsInMyvod = myvodSer.isMyVod("1", assetId, TypeEnum.EPISODE.toString());
					...
					Log.d(TAG, "AddMyvod mAssetId="+mAssetId);
					MyVodServiceImpl myvodSer = new MyVodServiceImpl();
					mMyVodAddResult = myvodSer.add( mChannelId, mAssetId, TypeEnum.EPISODE.toString());
					//[end]
			...
			if( mInfoVaild ){
				if( mServiceInfo != null ){
					...
					if( impl.getPlayStatus(offingId) ){
						// 已经在播放了，直接启动播放器
						EpisodeDetail.this.showPlayer( 
								offingId, 
								mProgram.get(KeyEnum.title.toString()).toString() + "#" + 
										mEpisodeListAdapter.getSelectItem().get(KeyEnum.episodeId.toString()).toString(),
			...
			if( mInfoVaild ){
				...
				if( it != null ){
					String title = mProgram.get(KeyEnum.title.toString()).toString() 
							+ "#" + it.get(KeyEnum.episodeId.toString()).toString();
					String type = TypeEnum.EPISODE.toString();
	...
	private void getTestInfo(){
		...
		for (int i = 0; i < mEpisodeCount; i++) {
			...
			map.put(KeyEnum.episodeId.toString(), i+1);
			list.add(map);
		}
